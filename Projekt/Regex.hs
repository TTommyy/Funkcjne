...
import Parsing (sleashChar, periodChar, anyDigit, anyNotDigit, anyAlpha,
...
import Handlers
    ( OperationType(ADD, ANY_DIGIT, ANY_NOT_DIGIT, ANY_ALPHA,
...
-- Then 2 args fun --
  | sleashChar h t = (ADD, ['\\'], tail t)
  | periodChar h t = (ADD, ".", tail t)
  ...
  | notOrGroup h t = handleNotOrGroup t
  -- and one args fun
  | escapeChar h = (ESCAPE, [h], t)
