...
-- (abc|def)	Matches abc or def NESTED () are unsuported due to every think may be achived trough (expr1|expr2|...|exprn)

module Operations(addToSet, addAnyToSet,
...
addNotORGroupToSet res group = concatMap (\c -> map (\w -> w ++[c]) res) (notGroup group)
addORRangeGroupToSet res range = concatMap (\c -> map (\w -> w ++[c]) res) (generateRange range)
...
addRangeRepetitionsToSet res toAdd (n, m) = concatMap (addRepetitionsToSet res toAdd) [n..m]
addOptionalToSet res toAdd = res ++ addToSet res toAdd

addMultipleRangeRepetitionsToSet res toAdd (0, 0) = res
addMultipleRangeRepetitionsToSet res toAdd (0, m) = res ++ addMultipleRangeRepetitionsToSet res toAdd (1, m)
addMultipleRangeRepetitionsToSet res toAdd (n, m) =
  concatMap (\c ->addMultipleRangeRepetitionsToSet (addToSet res [c]) toAdd (n-1, m-1)) toAdd